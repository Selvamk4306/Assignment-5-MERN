          ...
          data.slice(0, 30).map((product) => (
            ...
            >
              <img
                ...
                alt={product.title}
                className="rounded-md w-48 h-48 object-cover border mb-4"
              />
              <div className="w-full px-2">
                <h2 className="text-lg font-semibold text-gray-900 mb-2 truncate">
                  {product.title}
                </h2>
                <p className="text-sm text-gray-700 mb-1 truncate">{product.description}</p>
                <span className="block text-blue-700 font-bold text-center mb-2">₹{product.price}</span>
